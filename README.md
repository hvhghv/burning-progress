...
`rootfs pack` 会在创建临时归档前检查上述必要文件、运行配置和 handoff 入口，预检失败不会生成输出或 `.tmp.<pid>` 文件。命令会先把当前执行中的 `burning-progress` 写入 `rootfs/sbin/burning-progress` 再继续打包，因此不会被旧副本卡住。CPIO 使用未压缩的 `newc` 格式，大小通常接近 rootfs 内所有文件的总逻辑大小；若需要减小归档体积，应输出 `.tar.gz` 或使用 `--format tar.gz`。
